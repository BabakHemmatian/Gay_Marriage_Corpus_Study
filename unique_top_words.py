...
top_topics = ldam.ldamodel.top_topics
...
with open("top_words_all_"+str(num_topics),"r") as f:
    for idx,line in enumerate(f):
        ...
        if idx != 0 and idx % 2 != 0:
            top_word[idx / 2] = (ast.literal_eval(line)) # comment out the next
            # line if you want [topn] top words, not [topn]/2
            top_word[idx / 2] = top_word[idx / 2][:(len(top_word[idx / 2]) / 2 )- 1]

## Identify unique and almost unique top words of top topics
uniques = {key:[] for key in top_word.iterkeys()}
twos = {key:[] for key in top_word.iterkeys()}
other_top_words = {key:[] for key in top_word.iterkeys()}
for topic in top_word.iterkeys():
    if topic in top_topics:
        print (topic)
        for other_topic in top_word.iterkeys():
...
for idx,word in enumerate(all_top_words):
    if word[0] != "marriage" and word[0] != "gay":
...
print("mean (different topic): "+numpy.mean(all_non_assigned))
print("max (different topic): "+maximum_non_assigned)
print("min (different topic): "+numpy.amin(all_non_assigned))
print("standard deviation (different topic): "+numpy.std(all_non_assigned))
print("***")
print("mean (assigned top topic): "+numpy.mean(all_assigned))
print("max (assigned top topic): "+numpy.amax(all_assigned))
print("min (assigned top topic): "+numpy.amin(all_assigned))
print("standard deviation (assigned top topic): "+numpy.std(all_assigned))
