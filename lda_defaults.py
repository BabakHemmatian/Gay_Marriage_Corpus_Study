...
# threshold instead
top_topic_thresh = 0.03 # threshold for determining topics to report
top_topic_set = [4,14,22,23,27,28,47,12,29,48,49,16,33] # Choose specific topics
# to sample comments and top words from (default is topics reported in the paper)
# set to None to use threshold or fraction instead
...
# comparison. Set to None for no sampling. Needs prior parsing with write_original = True
...
regex=re.compile("(^(?=.*gay|.*\\bfag|.*faggot|.*fagot|.*queer|.*\\bhomo|.*LGBT|.*GLBT|.*same.sex|.*lesbian|.*\\bdike|.*\\bdyke|.*sodom)(?=.*marry|.*civil union|.*marri).*$)|(^(?=.*marriage equality|.*equal marriage).*$)", re.I)
